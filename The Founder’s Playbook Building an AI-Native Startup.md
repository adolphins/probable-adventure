...
* The startup life cycle, rebooted for 2026
...
## Chapter 1: The startup life cycle, rebooted for 2026

第1章：为2026年重塑的初创企业生命周期
...
这意味着在采取行动之前要具体化。“人们在费用报销上很挣扎”这只是一种观察。“中端市场公司的财务经理每周花费四个多小时来核对提交的报销单，因为他们当前的工具未能与其会计软件整合”，这才是一个可测试的假设。
...
*挑战：当技术障碍被消除时，充满激情的创始人可能会跳过初创之旅中最重要的工作：验证他们的想法是否真的是人们需要且会使用的解决方案。*
...
即使在如今智能体编程时代到来之前，**也有42%的初创公司因为做出了没人想要的东西而失败**。而现在，像 Claude Code 这样的智能体编程解决方案大幅缩短了从 *“我有一个想法”* 到 *“我有一个产品”* 的距离，这种失败率只会继续攀升。
...
虽然对于拥有绝妙想法的创始人来说，现在是最好的时代，但能快速且轻松地拼凑出一个看起来像产品的原型，违反直觉地，反而给 AI 原生初创公司带来了真正危险的生存风险。
...
直到最近，产品构建还需要实打实的开发时间和预算，即使是弄一个基础的原型通常也需要几个月。既然技术开发的障碍已基本消除，AI 让创始人太容易直接跳入构建阶段，而不去验证它在现实世界中的效用。
...
实现“问题-解决方案契合”需要先验证你的假设然后再构建，但许多首次创业（甚至经验丰富的）的创始人错误地认为，AI 可以绕过这个要求，将流程变成了：*有一个想法 -> 立即构建原型 -> 将原型的存在视为验证本身*。原型成了相信该假设一直都正确的理由，却从未测试它是否真的符合事实。
...
一个能运行的原型很容易被误认为是“你正在解决实际问题”的铁证，但事实并非如此。相反，你的原型应该作为在与潜在用户对话时进行压力测试的有用道具。这些对话本身才是真正的证据。
...
*挑战：当构建产品变得毫不费力且立竿见影时，你的执行规模可能会远远超前于业务的实际需求。*
...
过早扩张意味着在你真正验证某条产品路径值得投入之前，就已经向其投入了大量资源。
...
这向来是初创企业的致命杀手，但 AI 让创始人更容易在不知不觉中掉进过早扩张的陷阱。智能体编程助手太强大了，以至于很容易在验证“问题-解决方案契合度”之前就大幅扩展执行力，而你甚至都没有意识到自己已经偏离了航线。
...
它会围绕一个有根本缺陷的前提来生成、测试、调试和重构代码库，其热情与面对一个绝佳点子时毫无二致。系统中的真正智慧来源于你。现阶段的首要原则是：让你的“意义构建（逻辑判断）”走在“代码构建”的前面，尤其是当写代码变得如此快速且毫不费力的时候。
...
*挑战：要求 AI 工具寻找支持你已有信念的证据，它肯定能找到。现在，确认偏误配备了一个强大的搜索引擎。*
...
确认偏误一直是初创企业的一种职业风险：创始人生来就对自己的想法充满热情。现在，AI 工具为确认偏误提供了显著的增益。让 AI 验证你的创业想法，它会找到支持性的证据；让它估算你的潜在市场规模，它会找到能让你的总潜在市场（TAM）看起来值得投资的数字。
...
AI 是听从你指挥的，这意味着一个不提出尖锐问题的创始人，现在可以比以往任何时候都快地为一个坏主意炮制出一份详尽的、看似调研充分的论证，同时还满怀信心地认为自己实际上已经做了尽职调查。解药其实是同一个工具，只不过需要指向相反的方向：AI 在对一个想法进行压力测试时，能和它在验证该想法时一样彻底。
...
当研究和结构化的逆向思维暴露出你的想法需要修改的证据时，这就是转向（pivot）的信号。



### How Claude can help Idea stage founders

Claude 如何帮助处于创意阶段的创始人
...
推动你的 AI 原生初创概念度过创意阶段，感觉可能会无比漫长。你是个创始人，你只想动手做产品。但这个至关重要的起步阶段本质上是一个研究和验证的过程，这意味着在全力以赴写代码之前，你需要借助工具来帮助你更严谨地思考。以下是利用 Claude 的各项产品界面（Chat、Claude Cowork 和 Claude Code）来尽可能快地度过创意阶段，同时做好适当的尽职调查的方法。
...
AI 让初创公司创始人能更容易地快速交付、自动化繁琐的工作流以及规模化运营，但你使用的产品界面很关键。根据手头的任务，下面介绍了何时该使用 Chat、Claude Cowork 或 Claude Code。
...
* **Chat** 适合在不离开当前所处应用的情况下进行快速交流。用它来处理运营公司时不断出现的琐碎任务：从冗长的投资者备忘录中提取出一句话总结，在董事会会议前对某项声明进行合理性检查，或者理清团队里长长的 Slack 聊天记录。
...
* **Claude Cowork** 适用于真正需要耗费时间的知识型工作：从多个来源提取信息、理解这些信息并产出成品（如文档、演示文稿或电子表格）。比如，将一个文件夹的客户通话录音转写稿变成下一次产品审查的主题发现文档；在融资前利用十几个供应商网站的信息构建一份竞争格局分析；或者设置一个固定的周一早晨任务，从你连接的工具中提取指标，并将一份每周 KPI 简报自动放入共享文件夹。
...
* **Claude Code** 是为你的团队工程师提供的智能体编程环境：支持直接访问代码库、计划模式（Plan Mode）、Git 集成，以及本地、IDE 或沙盒云环境。在这里，精益团队可以在不断增长的代码库中交付功能，迁移 MVP 时期的遗留代码，并在不需要等待招聘更多人手的情况下，完成从原型到生产环境的跨越。

| If the task is...                                            | Reach for     | Why                                               |
| :----------------------------------------------------------- | :------------ | :------------------------------------------------ |
| **如果任务是...**                                            | **请使用**    | **为什么**                                        |
| A question, a rewrite, a quick brainstorm                    | Chat          | Fast, conversational, no setup                    |
| 问个问题、重写段落、快速头脑风暴                             | Chat          | 快速，对话式，无需任何配置                        |
...
你自身的领域专业知识和前期研究已经产生了一个假设。首要任务是不断打磨它，直到它真正具有可测试性。Claude 在强迫你做到“具体化”方面特别有用：究竟是谁有这个问题？多久发生一次？有多严重？他们目前是如何解决的？一个不能精确回答这些问题的问题陈述，还没有达到可以被验证的程度。
...
* **练习：** 与 Claude 一起打磨你的问题陈述，直到它成为一个可测试的假设。例如，*“合同审查时间太长”* 是无法进行有效测试的。但 *“中型企业内部法务团队在每个合同审查周期需要花费3天以上的时间，因为红线修改是通过冗长的电子邮件进行管理的，而不是在单一的版本控制文档中进行”*，这就是一个非常可测试的假设。
...
你的下一步是让 Claude 反驳你的想法，并寻找能够推翻你假设的相反证据。这可以暴露出负面的市场信号、失败的竞争对手、客户的行为模式以及结构性障碍，而一个总是顺着你意图去总结的分析往往会悄悄地淡化这些信息。
...
目标是：在进入“客户发现（Customer discovery）”阶段时，你的假设已经经受过最强有力的反面论点的压力测试，这样一来，信息收集性质的用户访谈才能是真正开放式的，而不是去刻意寻找印证。

*Note: Using Claude as structured devil’s advocate is a core use case at every stage of the AI startup life cycle.*
*注意：在 AI 初创企业生命周期的每个阶段，将 Claude 作为结构化的“魔鬼代言人（唱反调者）”都是一个核心的用例。*
...
初创公司有一种特有的现象叫“竞争对手忽视（competitor neglect）”：即倾向于过度关注自己的愿景和执行，从而系统性地低估了其他人在同一领域的动作。幸运的是，AI 提供了这剂解药：要求 Claude 给出最令人信服的论证，说明为什么在这个解决方案领域，某个竞争对手会成功，而你却会失败。
...
Claude 可以分析为什么他们的方法实际上更好、为什么客户会选择他们、为什么你潜在的差异化优势可能并不像你想象的那样站得住脚。
...
* **练习：** 要求 Claude 按层级绘制你的竞争格局：直接竞争对手、间接竞争对手、潜在收购方，以及可能进入你领域的相邻玩家。然后让它论证为什么每一层级都对你的成功构成了真正的威胁，而不是只给出最容易被驳倒的那些软弱说辞。
...
Claude Code 可以综合公开的客户反馈，以发现反复出现的投诉和未被满足的需求。额外好处是：这样做本质上就是对竞争对手客户进行免费的定性研究。
...
* **练习：** 指导 Claude Cowork 从关键来源综合竞争对手的用户评论，并找出痛点最高的几个现有解决方案尚未解决的抱怨。如果你的假设解决了其中一个或多个痛点，那就是证明“问题-解决方案契合”的有力证据。如果没有，那这也是值得提前知晓的。
...
Claude Cowork 还可以从密集的行业报告、分析师文件和市场研究文档中提取相关信息和数据；接下来，这些清晰、经过提炼的输入信息将成为 Claude 进行分析工作的理想上下文。
...
* **练习：** 基于公开数据构建 TAM/SAM/SOM（总潜在市场/可服务可触达市场/可获得市场）模型，并对背后的假设进行压力测试。识别该市场是处于扩张期、整合期还是成熟期；这个背景信息将影响你对入场时机和差异化策略的考量。绘制买家画像：谁掌握预算？谁影响决策？他们是同一个人吗？
...
最后，利用 Claude 寻找那些能告诉你入场时机是否正确的早期指标。追踪已经开始讨论你的问题的 Reddit 子版块和 LinkedIn 群组，并关注用户在描述他们的问题时使用的具体话术。要求 Claude 找出解决了类似问题的同类市场，并提取出哪些方法有效、哪些无效。挖掘可能加速或威胁这一市场机遇的监管、技术或人口趋势。
...
* **练习：** 要求 Claude 找出三个可能在未来两年内显著影响你所在市场的外部趋势（监管、技术或人口结构的），并评估每一个趋势对你的具体假设来说，是顺风还是逆风。
...
*注意：本节中的市场调研和竞争格局绘制工作并不是一次性的练习。在 MVP 和发布阶段，你将继续有所发现并不断演进你的想法，因此，每当你的假设发生变化时，重复这些练习非常重要。*
...
通过与潜在产品用户交谈所能学到东西的质量，取决于（1）你提出的问题的质量，以及（2）你是否问对了人。Claude 在进行客户发现时特别有帮助，包括该和谁谈、问什么，以及如何理解你听到的内容。
...
一个精确的目标画像比一份长长的联系人名单有价值得多，这包括最有可能强烈感受到该问题的具体职位、公司类型、团队结构和资历水平。从这些出发，找出这些人在哪里真正可以被触达到——他们聚集的社区、活动、LinkedIn群组和 Slack 工作区——并根据他们与该问题的密切程度，建立一个决定“先联系谁”的优先级框架。
...
确定了目标后，使用 Claude 来构建访谈框架本身：正确的问题，以正确的顺序排列，其结构设计旨在引出人们“实际是怎么做的”，而不是他们“认为自己会怎么做”。新手创始人常犯的错误是询问关于未来的宽泛、开放式的问题（*“你会使用类似这样的东西吗？”*），而不是具体地询问相关的过去经验（*“告诉我你上次处理这个问题时的情况。”*）
...
Claude 可以标记出你的草稿中哪些问题带有诱导性、过于宽泛，或者可能产生噪音而不是有效信号。Claude 还可以帮你设计追问，以探究被受访者转移的话题，或者针对重要问题的模糊回答进行深挖。
...
如果你的假设涉及到多个用户画像，Claude 也可以为每个人群设计不同的问题集。财务经理和首席财务官（CFO）与同一个问题的关系是不同的，单一的访谈框架会抹除这种区别。
...
* **练习：** 首先手动起草你的访谈问题，然后请 Claude 审查它们。特别要求它标记出任何带有诱导性、面向未来、过于宽泛，或可能导致受访者给出“符合社会期望的答案”而非诚实回答的问题。然后让它为访谈中最可能导致受访者转移话题的两三个关键点，建议一些后续的深挖问题。
...
每次对话后，使用 Claude 进行复盘：将你的笔记喂给它，要求它识别哪些内容证实了你的假设，哪些挑战了你的假设，以及哪些内容确实出人意料。当你收集了一批访谈后，通过 Claude Cowork 运行你的全部访谈笔记，以提炼出反复出现的主题、矛盾点，以及正反两个方向上最强烈的信号。然后，将这份合成输出带回给 Claude，要求它指出在你对数据的解读中，哪些地方可能只是在模式匹配“你想听到的东西”，而不是事实本身。
...
* **练习：** 每完成五次访谈，指示 Claude Cowork 综合你的笔记并生成两个列表：支持你假设的证据，以及挑战你假设的证据。如果第一个列表明显长于第二个列表，询问 Claude 这种不对称性究竟是反映了数据中的真实情况——还是仅仅反映了你希望找到的东西。
...
使用 Claude Cowork 来自动化构建联系人名单、执行外联以及安排用户访谈等繁琐的运营工作。
...
Claude Cowork 可以使用你与 Claude 一起定义的目标用户画像（包括职位、公司类型和资历水平）来研究并编制一份结构化的潜在客户名单及已验证的联系信息。然后，它会大规模起草个性化的外联邮件，并根据个人的角色和背景量身定制每一封邮件。
...
当收到回复时，它通过 MCP（模型上下文协议）连接到 Gmail 和 Google 日历，以管理邮件线程、处理日程安排请求，并将访谈添加到日历中。该工作流会持续进行，Claude Cowork 会按既定节奏生成跟进邮件的草稿（例如，针对尚未回复的联系人发送第七天跟进邮件），并在完成每一步时更新你的追踪表格，以便你始终清楚每位潜在客户在漏斗中的进度。
...
* **练习：** 将你验证过的访谈目标画像提供给 Claude Cowork，要求它建立一个潜在客户列表、起草个性化的外联邮件序列，并建立一个追踪表格（包含外联状态、跟进节奏和访谈完成情况的列）。然后让它去负责统筹协调，而你则专注于为对话本身做准备。
...
你已经完成了验证工作：问题是真实的，你知道谁有这个问题，并且你有一个证据支持的解决方案概念。现在使用 Claude 从各个角度来深化并挑战你的解决方案概念：还有哪些缺陷？存在哪些替代方案？为了使这个解决方案能在规模化时奏效，必须具备哪些前提条件？这是一个重要的现实检查点：这个设计是否真的解决了在验证过程中发现的问题，而不是你一开始假设的那个问题？
...
* **练习：** 向 Claude 展示你的解决方案概念，并要求它找出你的设计最依赖的三个假设。然后问问，要使每个假设成立必须具备什么条件，如果其中任何一个不成立会有什么后果。
...
现在到了有趣的部分了：有了经过验证的假设和经过压力测试的解决方案概念，你终于准备好可以动手构建了。
...
在这个创意阶段，是时候让 Claude Code 出场了。即使你一直都在鼓捣，现在也是你生成官方“轻量级原型”的时刻：只需极小的表面积即可，只要能将你的想法展示给真实的人并获得真实的反应即可。
...
你还不是在构建一个现实世界的产品；你正在构建一个你想法的功能样本，用于客户和投资者的对话。真实用户对他们实际能触摸到的东西做出的反应，会告诉你几十次问题发现访谈都无法告诉你的信息。以前，你在证明你要解决的问题是真实的；现在，你是在让潜在用户体验并参与到所提出的解决方案中。
...
* **练习：** 定义你的解决方案所依赖的单一核心交互。指示 Claude Code 仅构建该部分。当你完成后，将其放在五位符合验证目标画像的人面前，请他们试用。你从这五次对话中学到的东西，将决定你是继续构建，还是推倒重来。
...
走到创意阶段的尾声，是 AI 初创竞赛中的一次巨大飞跃，因为现在你不再依靠直觉下注；你是基于证据在执行。现在进入 MVP（最小可行性产品）阶段，创始人的指导问题将从“这值得构建吗？”转变为“我们究竟应该首先构建什么？”，而 AI 的主要角色也将从研究伙伴转变为施工队伍。
...
许多创始人将 MVP 阶段视为一个施工阶段，但 MVP 阶段从根本上说仍然是一个“收集证据”的演习。区别在于，你现在收集的是关于解决方案的证据，而不是问题空间；具体来说，就是是否存在一个真实的、可识别的人群，发现它足够有价值，以至于愿意使用它、回头继续使用、为它付费和/或把它推荐给其他人。
...
作为 AI 原生初创公司的创始人，你的目标是将**已验证的问题转化为真实用户实际会使用的工作产品**。这并不是包含路线图上所有功能的完整版本，而是你想法中最小、最聚焦的迭代版本，它将一个真实的解决方案摆在真实用户面前，并产生能够证明“产品-市场契合度（PMF）”的真实证据。
...
与此同时，你现在的构建方式决定了未来的可能性。这意味着 MVP 阶段还有第二个同等重要的目标：**快速行动的同时，不积累那种会产生复利的技术债务**——因为当真实用户大规模涌入时，这些技术债务会像幽灵一样缠着你。
...
最后，从第一天起就**投资于持久的上下文语境**，是让 AI 成为力量倍增器而非混乱之源的关键。在 AI 原生初创公司中，你的代码库是你日复一日与 AI 协作的产物，这就使得“可读性”成为基石。跳过编写规范、架构决策和上下文文件（如 `CLAUDE.md`）的创始人，必然会撞上一堵可见的墙：每一个新的对话周期都需要重新解释代码库，并且 AI 生成的更改将逐渐偏离最初的愿景。
...
MVP 阶段的退出条件是获得了“产品-市场契合度（PMF）”的真实证据：证明有一个特定的、可识别的用户群体觉得该产品足够有价值，愿意继续使用（留存率）、愿意付费（收入）或告诉他人（推荐）。
...
在 MVP 阶段，创始人的首要指令是速度和判断力。这里的挑战集中在：你是否能以正确的方式构建正确的产品；速度是否足够快以抓住市场；同时又没有为了走捷径而在未来付出惨痛代价。
...
*挑战：由于 AI 本质上消除了过去控制“什么能推向生产环境”的所有自然瓶颈，速度得到了保证。但是，当速度成为创始人构建 MVP 时唯一考虑的变量时，他们就面临着积累技术债务的风险，这债务日后将极难偿还。*
...
在 MVP 阶段，存在一些技术债务是合适的，只要你明白必须在规模化之前对其进行管理。它会逐渐累积，并可以随着时间推移或通过专门的冲刺计划（sprint）来清除。然而，AI 产生的技术债务，是呈复利增长的。
...
如果没有把规范和架构约束写在一个 AI 可以读取的地方，那么每一次的对话都会从头开始重新推导基础决策，这些决策就会产生偏移。最终，你会得到一个背后没有任何连贯心智模型的代码库，这并不是因为其中任何一个单一片段写得很糟糕，而是因为这些片段从一开始就没被设计成能拼凑在一起。这是一个真正的问题，而且它往往会在后期才暴露出来。
...
*挑战：AI 工具能够产生令人印象深刻的早期数据，但这并不能保证市场确实需要你的产品。*
...
早期的势头是创始人能体验到的在心理上最具有震撼力的经历之一。经过数周或数月的验证工作和谨慎、自律的开发，产品的发布让人感觉像是在确认你一直以来都是对的。
...
智能体编程工具可以帮助你比以往任何时候都快地到达这一时刻，但早期吸引力（traction）与产品-市场契合度（PMF）不是一回事。发布初期的能量往往产生于转瞬即逝的力量，比如你的创始人朋友们、你的投资者旗下其他投资组合公司里的潜在买家，或者是一条能带来流量飙升的 Hacker News 头条。不幸的是，当这种最初的推动力消退时，它们都无法可靠地预测在第六周或第十二周会发生什么。
...
*挑战：当开发感觉毫不费力且几乎没有成本时，总会想要“再加一个炫酷功能”或“再处理一个边缘情况”。这种范围蔓延往往弊大于利。*
...
范围蔓延一直都是初创公司的风险。现在的区别在于，过去对抗它的传统强制机制——工程时间的实际成本——已不复存在，因为现在添加一个功能只需要一个下午，而不是整整一个开发冲刺周期。
...
这里的挑战在于，每一个单独的添加似乎都是有道理的。产品当然应该处理那个边缘情况；用户当然会想要那个工作流。在当下，这些感觉并不像是范围蔓延，因为使用智能体编程工具构建每一个功能都太不费吹灰之力了，但是当你的产品向其原始边界之外蔓延时，你就面临着失去方向和势头的风险。
...
解药是在开发开始之前起草一份书面的“范围定义”，描述产品应该做什么，并明确指出它不做什么；同时规定，若要添加新功能，必须提供来自真实用户的具体证据。这将决策点从“我们应该构建这个吗？”转变为“是否有大量核心用户告诉我们，没有这个功能他们就无法从产品中获得价值？”。
...
*挑战：使用 AI 工具急于将应用程序推向市场的创始人，如果在没有事先了解基本安全原则的情况下这么做，最终会使他们的用户暴露在原本可以预防的风险之中。*
...
残酷的事实是，智能体编程工具生成的是“能运行的代码”，而不是“天生安全的代码”。功能性代码很容易验证，因为功能要么好用要么不好用。而安全漏洞在被利用之前是隐形的，这意味着对于首次创业的创始人来说，没有自然的反馈循环来警告他们出问题了。然而，向真实用户发布一个在线 MVP 意味着面临真实的数据、真实的暴露风险，并且如果出了差错，还会造成真实的后果。
...
轻视安全性对于 AI 原生项目来说并不是什么新鲜事。各个时代的自举型（Bootstrapped）初创公司通常都会将安全考虑推迟到开发的后期，有时甚至等到临近产品发布时才考虑。但在任何用户接触到你的应用程序或解决方案之前进行一次安全审查，是将最小可行性产品发布到世界上的最低责任门槛。



### How Claude can help MVP stage founders

Claude 如何帮助处于 MVP 阶段的创始人
...
在 Claude Code 写下一行生产代码之前，先使用 Claude 来定义并记录将在这一阶段支配所有构建工作的架构决策：要遵循的模式、要避免的依赖项、所做的权衡以及原因。此输出将作为一个聚焦的架构上下文文档，并为 Claude Code 建立操作的护栏。
...
如果没有这个上下文，每个对话都会从头开始，Claude Code 将被迫推断它自己的结构假设。让 Claude Code 在没有护栏的情况下进行构建，会产生一个功能可行但结构不连贯的代码库，而在不连贯的代码库上进行迭代和扩展，最终都是在浪费时间和 Token。迟早有一天，代码会不可避免地崩溃，迫使你从头开始重建。
...
* **练习：** 在打开 Claude Code 之前，先打开 Claude 并描述你正在构建的东西：它解决的核心问题、它服务的用户，以及你在未来六个月内切合实际的预期规模。请它帮你定义应该支配 MVP 构建的架构原则，在给定限制下应避免的依赖项，以及你在这个阶段有意识接受的权衡。
...
接下来，**将此输出保存为 `CLAUDE.md` markdown 文件**。这是你的架构上下文文档：你构建的第一个产物，也是后续每次对话所依赖的基础。`CLAUDE.md` 文件作为 Claude Code 的项目级指令，提供了项目特定的上下文和指导，当 Agent SDK 在该目录中运行时会自动读取。从功能上讲，它们就是你项目的持久“记忆”。
...
无摩擦的范围蔓延是 AI 时代 MVP 最典型的失败模式之一。就像你定义并记录产品的应用架构一样，在开发哪怕一个单一功能之前，你也需要定义你的 MVP 范围。
...
Claude 可以帮助你创建一份范围说明文档，描述你的 MVP 产品做什么、故意不做什么，以及功能修改的标准：究竟来自真实用户的哪些具体证据，才足以成为在现阶段添加新内容的合理理由。
...
当新功能的想法浮现时——它们肯定会出现——你可以使用 Claude 进行压力测试，判断它究竟是来自用户的真实信号，还是披着产品思维外衣的创始人个人热情。
...
一旦定义了架构和范围，Claude Code 就会成为主要的 MVP 构建工具。使用它来生成、测试、调试并迭代你的代码库，但请将每次对话视为对你“已经做出的产品决策”的执行，而不是借机塞入一些新点子的机会。
...
每次开启 Claude Code 对话时，先（1）重温你的范围说明文档，（2）为模型提供你的 `CLAUDE.md` 架构上下文文档。每次对话结束时，用本次对话中浮现的任何新决策来更新它。目标是拥有一个你能够解释其结构的代码库，而不仅仅是一个能跑起来的代码库。
...
* **练习：** 为你的 Claude Code 工作创建一个简单的对话模板，其中包含架构上下文文档、本次会话的具体任务以及需要遵守的任何约束或模式。在每次对话结束时，向上下文文档添加一份简短的日志记录，详细说明构建了什么、做出了什么决策以及该次对话引入了哪些假设。每次花五分钟记录文档，是防止架构漂移（最终复利演变成难以管理的代码库）的廉价保险。
...
作为 AI 原生初创公司的创始人，你的责任是了解你的代码库里有什么、理解任何潜在的风险暴露途径，并且绝对不要把明显的漏洞发布给那些信任你并把数据交给你的真实用户。
...
Claude 可以对 AI 生成的代码进行有用的初步安全审查，并帮助识别常见漏洞。将这养成在发布前加入循环的一个好习惯。然而，它并不能替代专业的安全工具，或者在风险更高的情况下，不能替代人类审查员——那些将其视为唯一护身符的创始人，最终往往成了安全漏洞新闻里的主角。
...
**Claude Code Security** 则更进一步：它会扫描代码库寻找安全漏洞，并提供有针对性的补丁建议供人工审核，这能暴露出传统方法可能会漏掉的问题。
...
*注意：在本书出版时，Claude Code Security 尚处于有限内测（limited beta）阶段，因此在将其纳入工作流之前请检查其可用性。*
...
* **练习：** 在向任何真实用户部署之前，带着明确的指令让 Claude 检查你的核心应用代码：审查身份验证和会话处理、API 响应中的数据暴露情况、输入验证和注入风险，以及是否存在带有已知漏洞的依赖项。严肃对待每一个发现并评估是否需要修复，任何涉及身份验证、密钥或数据处理的内容都必须经过人工复核。
...
那些错误地将早期吸引力认作“产品-市场契合度（PMF）”的创始人，通常也是那些在产品发布后才开始跟踪数据的人，他们选择的指标是为了评估“哪些方面做得好”，而不是为了暴露“哪些方面做得差”。解药是在第一位用户出现之前，就建立好你的指标衡量框架。
...
使用 Claude 来定义哪些指标对你的特定产品至关重要、基准线是什么，以及数据中的哪些模式才能构成真正的“产品-市场契合”，而不是自我陶醉的噪音。具体来说：在发布 MVP 之前，设定好你的留存基准、激活标准，以及第7天和第30天的目标。
...
接下来，定义对你的特定产品来说，什么样的数据是“假阳性（false positive）”：例如，有注册但无激活、有收入但无留存，或者有最初的热情却没有重复使用。当数据出来时，要求 Claude 针对你自己的产品吸引力做逆向推理：一个怀疑论者会对这些数字怎么看？
...
一旦真实用户进入产品，运营层面的工作就会迅速扩张。Claude Cowork 能够处理重要但繁琐的工作，如建立和维护用户联系人名单、执行外联序列、安排反馈会议、分诊（triage）Bug 报告以及追踪迭代周期。在创意阶段用于管理客户发现运营工作的 MCP 协议集成，在这里同样适用。
...
在反馈收集环节保持“人类介入（Human in the loop）”，以便对用户反馈进行细微的探索。例如，当用户说“这很棒，但我希望它也能...”时，就需要解释：这是核心需求还是可有可无的（nice-to-have）？是仅仅针对这个客户，还是代表了整个细分市场？确实是缺了这个功能，还是在早期新手引导环节（onboarding）出了问题？没有任何工具能直接回答这些问题。
...
* **练习：** 配置 Claude Cowork 运行你的 MVP 阶段反馈循环：起草向早期用户名单发送的邮件、安排反馈会议、为 Bug 报告和功能请求设计结构化的接收流程，并撰写每周收到的反馈的综合报告。先自己审查这份综合报告；之后，你可以要求 Claude 分析这些信息，以抓住你可能遗漏的任何重要观点。
...
MVP 阶段的结束标志是：当你拥有了真正证明“产品-市场契合度（PMF）”的证据，无论这产品感觉起来有多“未完成”。宣布你已经实现了 PMF 并准备好从 MVP 阶段进入发布阶段，最终是一个结合了创始人直觉和所收集证据的判断过程。不过，有一些有用的试金石测试可以参考：
...
* **肖恩·埃利斯测试（Sean Ellis test）：** 问问你的活跃用户：“如果你再也不能使用这个产品，你会有什么感觉？”如果超过40%的人回答“非常失望”，那就是一个有意义的 PMF 指标。
...
* **努力程度测试：** 在达到 PMF 之前，保持留存率需要不断的干预，包括频繁的主动联系、激励措施、亲自跟进，以及创始人耗费巨大的精力来保持用户参与度。在达到 PMF 之后，产品本身就开始承担这项工作。当事情开始从“推（push）”变成“拉（pull）”时，这种努力方向的转变是表明发生了真正质变的最清晰的信号之一。
...
最终，没有任何单一的数据点能绝对确认 PMF，因为它是一种必须在多个迭代周期内保持稳定的模式，然后你才能确切地宣布它的到来。
...
如果即使投入了这么多工作，你似乎仍无法实现“产品-市场契合度（PMF）”该怎么办？事实上，结果没有证实你最初的方向，这并不是失败，而是系统在发挥作用：MVP 阶段的使命就是在你为一个错误的答案过度投资之前暴露这些信息。

When the data doesn't support your current product, use Claude to work through what that data is telling you.
当数据不支持你当前的产品时，利用 Claude 帮助你理清这些数据到底在诉说什么。
...
* **探索替代的客户群体。** 也许那些没有转化的用户，从一开始就不是正确的目标受众。通常正确的受众已经在你的数据中了，只是权重被低估了。
...
* **调整你产品的价值主张（value prop）。** 也许你找对了受众，但你的 MVP 只是没能引起用户的共鸣。对上手引导（onboarding）、营销信息传达或核心功能侧重点的调整，有可能在不改变你已构建内容的情况下解决这个问题。

Stay open to the possibility that the disconnect may run deep enough to require a more fundamental change
保持开放的心态，这种脱节可能根深蒂固，以至于需要进行更根本的改变。
...
* **练习：** 如果你已经完成了三个或更多的迭代周期，但仍未向你的 PMF 基准取得有意义的进展，在决定下一步做什么之前，请使用 Claude 运行一次诊断。将你的留存数据、用户反馈以及最初的问题假设输入给它，并问它三个问题：
    * Is there a segment in this data responding differently than the rest?
    * 在这些数据中，是否有一个细分群体的反应与其他人不同？
    * Is the gap between designed value and experienced value a positioning problem or a product problem?
    * “设计价值”与“体验价值”之间的差距，是一个定位问题，还是产品本身的问题？
    ...
    * 为了让当前的产品找到真正的 PMF，必须满足哪些条件？考虑到你现在看到的情况，这种情况现实吗？

Let the answers determine whether you adjust, pivot, or return to the Idea stage.
让这些答案来决定你是该微调、果断转向（pivot），还是退回到创意阶段。
...
如果说 MVP 阶段是为了证明你的产品有存在的价值，那么发布阶段则是为了证明你的业务有增长的资格。
...
在发布阶段，初创企业创始人必须**将早期的吸引力转化为可重复、可持续的增长引擎**。除了使你的产品达到生产就绪状态外，你还必须强化其底层的基础设施，同时围绕你的产品建立一个真正的公司。
...
在创意和 MVP 阶段，初创公司自然是以创始人为中心的，因为你需要全方位的态势感知和紧密的反馈循环。但是现在，仍然试图亲自掌控每一条线索的创始人将成为发布阶段的瓶颈。目标并不是将你从公司中剔除，而是**构建运营系统以释放你的注意力**，让你去制定只有创始人才能做出的决策。
...
1. **增长是可重复且由渠道驱动的。** 你不仅在留住用户，你还在通过特定的渠道以可预测的方式获取他们，并且理解其单位经济效益：获客成本（CAC）、客户终身价值（LTV）和投资回收期（payback period）这些数字你都烂熟于心，并能为其辩护。
...
2. **产品可以处理生产级的工作负载。** 基础设施已加固，安全和合规性井然有序，可靠性在真实的生产环境（而不仅仅是你测试的环境）下能够保持。
...
3. **运营运转没有创始人造成的瓶颈。** 流程已建立，自动化已到位。你不再是那个亲自处理客户支持、问题分诊、冲刺计划或报告汇总的人。
...
寻找 PMF 是初创生命周期早期最难的问题。现在，创始人的挑战变成了保持它。发布阶段是许多找到了真实产品吸引力，却因为围绕和支持该产品的组织架构跟不上而依然分崩离析的危险期。以下是需要警惕的失败模式。
...
*挑战：为追求速度和验证而构建的 MVP 代码库运行得足够好，证明了产品是可行的，但现在，生产环境的流量、新功能的增加和不断增长的复杂性正在暴露出当初走的捷径。*
...
在 MVP 阶段，为了追求速度而积累一些技术债务是合理的权衡。在发布阶段，这笔债务开始产生利息，拖延不处理的时间越长，修复它的成本就越昂贵。
...
解决方案包括：进行系统性的架构审计以识别结构性弱点，进行有针对性的重构以解决其中最糟糕的问题，并有意义地扩大测试覆盖率，以便下一轮的功能开发不会再次引入相同的问题。
...
*挑战：在 MVP 阶段，创始人参与到每个环节是一项资产。在发布阶段，随着支持工单量的增加、产品决策的堆积以及运营复杂性的成倍增长，同样的本能却成为了制约因素。*
...
从“亲自做事”过渡到“设计替你做事的系统”，是初创生命周期中最艰难的转变之一。因为很少有一个明确的转折点，你面临的风险是完全错过这个时机，停留在“构建者模式”，而你周围的组织却陷入停滞。这种情况发生的明显迹象包括：原本一个小时就能做出的决定现在需要一周你才能顾得上；客户支持请求堆积如山，因为只有你知道答案；日常运营任务只有在你亲自想起去做时才会被执行。
...
补救措施是对你亲自处理的所有事务进行全面的盘点，从最微小的任务到最关键的决策，以确定哪些可以被系统化，哪些可以被委派，以及哪些仍然真正值得创始人投入时间和精力。
...
*挑战：在 MVP 阶段保持简单粗糙的安全和合规措施是没问题的，但现在，面对真实用户、真实数据以及潜在的企业合同，它就变成了一种累赘。*
...
在 MVP 阶段，只有少数测试用户且生产环境中没有敏感数据，安全漏洞只是理论上的风险。然而，当你的产品进入生产环境且真实用户开始依赖它时，这种假设就会变成非常真实的暴露风险。此外，那些不适用于原型的合规要求，在你处理客户数据、处理支付交易或向受监管的行业销售产品时，无疑也开始适用了。
...
补救措施是在生产规模到来之前（而不是之后）进行系统的安全和合规审查，并在下一波用户到来之前，将暴露出的所有问题视为“必须修复项”——而不是建议。
...
*挑战：新市场和融资机会看起来像是增长机会。但它们也可能是产品-市场契合度走向死亡的坟墓。*
...
你建立的初始吸引力是真实的，但它也特定于你的早期受众。过早地扩展到一个与你原始市场存在显著差异的市场，会引入新的用户行为、合规要求、支付基础设施，以及你的产品在设计之初并未考虑的基本期望。突然间出现了太多新变量，你失去了清晰解释自己数据的能力。在追逐一个新的、未经验证的受众时，你还面临着忽视原始用户群的风险。



### How Claude can help Launch stage founders

Claude 如何帮助处于发布阶段的创始人
...
Claude 的三种形态在发布阶段都得到了充分的使用，它们相互支持：每个工具产生的输出都会成为另外两个工具的输入。这种效果有机地叠加，同时使用这三种工具的创始人所获得的将远超其部分之和（1+1+1>3）。
...
这就是超精益初创模式在结构上成为可能的原因。当 Claude Code 负责构建产品、Claude Cowork 负责构建围绕该产品的公司、而 Claude 负责帮助运营这一产品和组织知识时，一个小团队就能像规模是其 N 倍的公司一样运转。
...
你的 MVP 代码库是可以运行的，但它也需要进行一次系统性的补救扫描，以寻找任何可能演变为结构性隐患的技术债务。
...
首先，使用 Claude Code 运行一次全面的架构审计：找出代码库中脆弱的地方、任何维护成本将变得高昂的走捷径行为，以及测试覆盖率薄弱到可能在下一轮新功能开发时重新引入相同问题的地方。
...
将 Claude Code 的审计发现反馈给 Claude，以对补救工作进行分诊（triage）和排序：哪些必须在下一个版本发布前修复，哪些可以推迟一个冲刺周期（sprint），以及鉴于你当前的阶段，哪些是可以接受的持续债务。这也是记录你在 MVP 阶段所做架构决策的绝佳时机（就是那些因为没有时间写下来而一直存在你脑子里的东西）。现在将它们写入 `CLAUDE.md` 可以确保未来每次 Claude Code 对话都从对“系统是如何设计的以及为什么这样设计”的共识开始。
...
* **练习：** 指示 Claude Code 审计你的 MVP 代码库，并生成一份优先级排序列表，包含结构性弱点、测试覆盖率缺口和重构候选对象。然后将该列表交给 Claude，要求它在你的几个开发冲刺周期中对补救工作进行排序：你需要首先解决的任何重大问题、可以与功能开发并行处理的事务，以及可以暂时搁置的事项。
...
为了建立起能解放你注意力的运营系统，让你去处理只有创始人才能应对的责任，你首先需要清楚地知道你的注意力都去哪儿了。使用 Claude Cowork 对你当前的运营负载进行一次结构化的审计，记录下每一个重复性任务、每一个落到你办公桌上的决策，以及每一个只有靠你个人记忆才会发生的工作流。然后让 Claude Cowork 将这份清单分类为：完全可以自动化的事项、需要人工干预但不必是你本人的事项，以及真正需要创始人判断的事项。
...
审计完成后，使用 Claude Cowork 为自动化的候选对象设计工作流逻辑：什么事件触发每个工作流，决策规则是什么，输出结果是什么样子的，以及完成后该输出送往何处。
...
使用 Claude Code 找出在目标市场所要求的 SOC 2、GDPR 或 HIPAA 审计和标准中经常出现的代码级问题。这将同时暴露出安全漏洞和合规差距。将这些发现喂给 Claude，让它帮助你确定补救工作的优先级，并设计企业买家在签约前必然会要求的控制措施、审计日志和访问权限管理。*注意：AI 扫描是一种辅助手段，不能替代合格的专业合规审查。*
...
接下来，将合规工作流整合进你的开发周期中，而不是将其视为一个一次性项目；合规文档需要不断维护和更新。对于那些即将面临企业合同或国际市场的创始人来说，这也是 Claude Code 安全扫描可以帮助你为独立安全评估做准备的最佳时刻。
...
* **练习：** 使用 Claude Code 运行一次面向你的目标市场所要求框架的代码级安全审查。将输出反馈给 Claude，要求它生成两样东西：一个有优先级的安全补救顺序列表，以及一份为了满足潜在企业买家合规审查你所需提供的文档和控制措施清单。
...
发布阶段需要一套轻量级、可重复的流程，这些流程无需创始人的干预就能触发或运转。使用 Claude 来设计：将如何构建你的产品时间线和工作周期，在 Claude Code 接触一个新功能之前产品需求文档（spec）需要包含什么，Bug 报告如何分流和路由，以及你的每周指标报告涵盖哪些内容、如何分发。
...
一旦流程设计完成，使用 Claude Cowork 构建并运行操作层：安排冲刺（sprint）会议仪式，将收到的 Bug 报告路由到正确的地方，从你连接的数据源汇总每周指标，并维持反馈循环以确保用户信号能源源不断地流入产品决策。
...
* **练习：** 要求 Claude 设计一个轻量级的产品管理操作系统：明确的冲刺节奏、最简的需求文档模板、Bug 分诊决策树，以及一份从你的实际数据源提取数据的每周指标简报。然后设置 Claude Cowork 来实施并运行该系统的重复性操作元素（如日程安排、路由分发和报告汇编），让它们在你不在场的情况下按时发生。
...
在扩展（Scale）阶段，创始人的角色将从构建者重新聚焦为面向公众的高管。产品仍然是核心，但你个人的日常工作将越来越围绕公司本身展开。即使你在努力维持精益的、以 AI 为中心的结构优势，你的注意力也必须扩展到全新的扩展阶段活动上，比如分析师简报和 IPO 路演。
...
扩展技术基础设施的工作仍在继续，现在又加入了扩展组织本身并使其成熟为一家企业的工作。
...
在扩展阶段，你的目标是从几千个用户增长到几百万个，从单一市场进军到多个市场。在之前的每一个阶段，增长都是一种你可以通过贴近用户、根据紧密反馈循环中的数据并辅以大量的创始人直觉来摸索前行的事情。但现在，目标是**建立系统性增长**，这种增长是由成熟的组织运营所支撑的。
...
对于一家 AI 原生初创公司来说，你的目标应该是**通过深厚的积累建立一条坚固的护城河**，这源于你融入产品的专业知识、你的产品与用户依赖的其他工具和平台的集成深度，以及专有的系统数据和工作流。那些一直在同一方向、在一致的基础设施上持续构建的创始人，现在拥有了一些真正难以复制的东西。
...
在这个阶段，公开市场投资者、分析师、监管机构、企业采购团队和潜在收购方将施加更大的压力——伴随着更深的怀疑——因为现在的风险更高了。**你的产品和组织必须经得起外部审查：** 不仅是你所构建的产品的能力，还包括围绕它的治理结构、合规姿态、财务控制和战略叙事。
...
扩展阶段的退出条件不再是一个单一的里程碑，而是一个阈值事件：即便创始人越来越不直接参与日常运营，公司也是可持续的。你已经展示了系统性的增长；建立了满足最苛刻的外部审查者的组织治理和合规基础设施；并且能够对这个问题给出一个坚实的答案：“如果今天有一家资金雄厚的巨头抄袭了你的产品，你的用户会留下来吗？”
...
在实践中，这个阈值通常采取三种形式之一：在不再需要外部资本的规模上实现可持续盈利、准备好 IPO，或被收购。这三者都要求你的增长是系统化且可审计的，你的产品护城河经得起审查，并且你的组织在运营上是成熟且可持续的。

When this is true, congratulations are in order: your startup has gone from being a bet to being a business.
当这些条件都满足时，应该恭喜你：你的初创公司已经从一场“赌注”变成了一门真正的“生意”。
...
*挑战：扩展阶段的运营系统必须在无人当“保姆”照看的情况下，可靠且可持续地运行。对于从第一天起就亲力亲为的创始人来说，这种转变在心理上的挑战丝毫不亚于结构上的挑战。*
...
你在发布阶段的工作是创建系统；到了扩展阶段，工作变成了（1）使这些系统成熟直到它们完全值得信赖，并且（2）真正去信任它们。
...
这比听起来要难。即使你是一个善于授权的创始人，什么是该交接的、什么是该留在自己盘子里的，也并不总是显而易见的。交接得太多、太快——尤其是交给 AI 自动化系统——关键的决策就会在缺乏“只有创始人才能提供的关键上下文”的情况下被做出。但是紧抓不放太久，你又会成为瓶颈。
...
这里的核心挑战是：识别那些只存在于创始人大脑中或未被记录的工作流中的“机构知识”，然后将其编纂到成文的、可审计的、可转移的系统中。
...
*挑战：客户不再仅仅评估你的产品；他们想知道你的组织是否能成为一个可靠的基础设施合作伙伴。*
...
前三个初创阶段的技术挑战都集中在代码库上：在不积累技术债务的情况下构建正确的解决方案，然后为真实用户强化安全和合规性。进入扩展阶段后，现在的挑战变成了围绕代码库建立的一切事物；创建支持基础设施、文档以及标志着成熟度的可靠性保障。
...
签订多年合同的大型客户和机构买家在签字之前就会要求这些保障，而且一旦签字，他们也会严格要求你兑现。不过，让你走到今天这一步的同一套 AI 基础设施，也能帮你建立具有明确响应时间的专属支持功能，以及新客户工程师团队能够真正看懂并使用的文档。
...
*挑战：处于扩展阶段的公司通常需要招聘、薪酬、会计和法律运营等组织层面的基础设施，这与公司到底由多少人在运营无关。*
...
在发布阶段，“系统化运营”意味着自动化那些消耗创始人注意力的工作流。而现在，一个处于扩展阶段的初创公司需要发展出更广泛、在某些方面更重要的运营职能，例如财务报告、合规监控、合同管理和客户支持等等。
...
*挑战：有机增长是有天花板的，大多数处于扩展阶段的创始人，在他们还未真正建立起一个成熟的进入市场（GTM）部门之前，就已经撞上了这个天花板。*
...
创意、MVP 和发布阶段的增长通常源于创始人主导的销售，从一次时机精准的 Product Hunt 发布到与早期客户的私人关系。然而，像这样的有机增长只在一定程度上有效，大多数初创公司在扩展阶段都会触及这个极限。迹象包括：用户增长曲线变平、获客成本上升，以及销售管道只有在创始人亲自介入时才会向前推进。
...
扩展阶段的增长需要构建一个专门的增长引擎，以触及更广泛的全新受众群体。然而，大多数初创企业创始人以前可能从未亲自运营过营销、销售和分析师关系项目。一个正规的 GTM 运作不仅需要建立新的系统和流程，还需要创造出一种品牌基调和故事，来表达你想如何谈论你的产品。因为在初创企业生命周期的这个阶段，你需要这样一个故事去打动的人不再仅仅是个别的初创用户，而是面向整个目标群体，如投资者和企业买家。
...
幸运的是，GTM 部门并不需要庞大才能有效，曾经帮助你构建产品的同一套 AI 基础设施，也能助你以低成本将其推向市场。
...
早期的初创阶段将 Claude 用作产品本身的基础设施：用于验证想法的研究伙伴、设计和构建原型的工程团队，以及让“单人初创公司”成为可能的 AI 运营层。进入扩展阶段的 AI 原生创始人现在可以使用 Claude、Claude Code 和 Claude Cowork，以他们当初构建产品相同的方式继续扩展公司。
...
以清晰的眼光开启扩展阶段，审视你现在最需要将时间和注意力投入到哪里，这对于从未建立过企业的首次创始人来说可能是一个挑战。Claude 可以帮你列出一份在此阶段“只有你才应该做的事情”的清单，这可能包括：决定产品叙事、维护董事会关系、企业级大型交易，以及创始人与创始人之间的对话。任何不在该清单上的事情，都是委派或通过 Claude Cowork 实现自动化的候选对象。
...
* **练习：** 使用 Claude 绘制一张你当前运营层面的“瓶颈地图”：每一个目前需要经过你的工作流、决策和审批。现在，让 Claude 推演一下：当你离开一周时，这些事情每一项会发生什么。那些陷入停滞的工作流，就是你仍然干涉过多，以至于会拖累进度的地方。

How do these map to the inventory of founder priorities and responsibilities you made with Claude?
这些发现如何映射到你和 Claude 共同制定的创始人优先级和职责清单中？
...
接下来，是时候对你已经建立的系统进行压力测试了，看看它们是否真的准备好随着业务的增长而进行扩展。
...
* **练习：** 使用 Claude 映射出你当前的工作流，然后问它当你离开一周时每一个会发生什么。陷入停滞的工作流，说明其交接标准、升级路径或异常处理机制还需要强化。Claude 可以帮助分析故障点并建议适当的修复方案，以便你可以根据需要更新或替换 Claude Cowork 的自动化流程。
...
随着规模的扩大，买家需要你让他们吃一颗定心丸：证明你的产品和组织可以作为长期基础设施被信赖。技术工作一如既往地在代码库内部进行，但现在，还有围绕代码库开展的技术工作需要处理。
...
第一步是将“机构知识”转化为可扩展的系统。使用 Claude 起草并维护企业采购部门期望看到的书面基础设施，包括产品文档、技术支持操作手册（playbooks）以及服务水平协议（SLA）。
...
与此同时，指导 Claude Code 根据企业合同要求的具体可靠性和安全标准来审计并加固代码库，并构建出那些基于 Discord 社区进行支持时从未被要求提供的技术支持基础设施：日志记录、监控、事件响应工具，以及使 SLA 真正具备可执行性的可观测性层面。
...
随后，由 Claude Cowork 自身来运行企业技术支持的操作层：工单路由、升级工作流、由产品变更触发的文档更新、续约跟踪，以及企业客户成功团队所依赖的定期报告。这三者的结合，赋予了一个小团队堪比大型组织的支持姿态，而这正是签订多年企业合同所要求你展现出的能力。
...
* **练习：** 挑选三个对你要求最苛刻的潜在客户，或者找出三个你最想签下的理想客户。要求 Claude 出具一份差距分析：这些客户公司的企业采购团队在签署多年合同之前期望看到什么样的文档、SLA 和支持基础设施？而你目前在哪里存在不足？利用该输出在 Claude Code 和 Claude Cowork 中对技术和文档工作进行排序。
...
创始人的拼搏精神带你走到了今天，但扩展初创公司需要创建并实施真正的进入市场（GTM）策略。AI 可以帮你建立并运行那台完整的 GTM 引擎。
...
Claude 可以协助从零开始构建基础的 GTM 资源：市场细分、信息传达架构、分析师关系策略、销售手册，以及当你面对公众投资者、企业买家和华尔街分析师时至关重要的“面向投资者的指标叙事”。这些受众中每一个都有其专属的词汇表，并根据其自身的标准来评估你；Claude 的工作就是将你产品的价值主张转化为与每个受众群体相关的产品营销策略。
...
现在，Claude Cowork 可以成为你的战术执行层：内容管道、出海/外呼序列、分析师简报后勤、新闻发布室与公关节奏、CRM 数据卫生、销售漏斗报告，以及将 GTM 策略转化为实际商业动作的众多重复周期。
...
凡是 GTM 动作需要产品营销基础设施的地方——交互式演示环境、集成文档、沙盒租户、API 参考、技术说明一页纸——Claude Code 都能为你构建。买家希望从技术上评估你的产品，在扩展阶段，仅仅一个 Loom 视频和一份销售演示文稿已经不够了。这也是能让你的 GTM 行动异步运行的基础设施：当你在开董事会的时候，一个构建良好的演示环境可能正在帮你促成交易。



#### Turning domain expertise and institutional knowledge into AI context

将领域专长和机构知识转化为 AI 上下文
...
许多极度精益的初创创始人正在为一个现实世界的问题构建高度专业化的应用或工具，这是他们在特定行业中亲身经历或直接观察到的。智能体 AI 现在让从未写过代码的创始人能够利用其领域知识构建产品来解决复杂问题。Claude、Claude Code 和 Claude Cowork 在将创始人的知识转化为能够产生复利的“产品专业性”方面，各司其职。
...
使用 Claude 捕获、组织并提炼创始人的知识，就能将领域专长放到产品可以触及的地方。通过长时间的对话、项目和记忆功能，创始人可以分享他们所知道的一切——行业术语、监管陷阱、边缘情况、挫折，以及为什么这个问题的常见答案行不通的原因——将其转化为结构化、可搜索的上下文。随后，**能力扩展（Skills）** 可以将重复性的工作流（例如，“我如何审查一份商业租赁合同”、“我如何分诊一份病患入院登记表”）编码成 Claude 每次都能以相同方式运行的可重复利用的程序。经过几个月的积累，这会变成任何通用型 AI 都无法比拟的专有知识底座。
...
利用 Claude 将你的领域知识外部化，对于将行业特定的边缘情况编码到你的产品中具有不可估量的价值：例如，一个通用型的 AI 医疗计费工具在处理 340B 药品福利计划报销时会崩溃，但你的工具却有针对它的特定逻辑。Claude Code 帮助你将行业内其他专业人士经常遇到的挫折，转化为验证逻辑、提示词优化，或者是一个与竞争对手听都没听过的利基行业系统建立的 MCP 协议集成。结果是，你的应用或工具的深度和广度都在以一种竞争对手根本无法复制的方式不断复合增长。
...
* **练习：** 在你的垂直领域找出一个通用型竞争对手肯定会做错的边缘情况。与 Claude Code 合作，根据你亲眼见过的场景为其建立一个专用的测试用例（不是普通的单元测试）。每次出现类似的边缘情况时，就把它加进去。你的测试套件最终会变成你护城河的地图。
...
当用户与你的产品交互时，他们会产生行为信号（即他们接受了哪些输出，拒绝了哪些输出），这为产品路线图提供了依据。随着时间的推移，你将了解到你特定用户群的具体模式、偏好和边缘情况。这就是我们所说的“复利价值”：每一项改进都使产品变得更有用，这会推动更多的使用，产生更多的反馈，从而推动更多的改进。
...
这些数据是被时间锁定（time-locked）的、特定于上下文的，并且是山寨者不可能重现的：你根本无法花钱买到成千上万在你的产品中打磨工作流的用户的“行为指纹”。
...
Claude 可以帮助审查你收集到的所有用户交互数据，识别其中信号最强的行为模式，并设计反馈循环，将持续的使用行为转化为系统性的模型优化。
...
* **练习：** 将你产品的交互数据摘要喂给 Claude：你一直在收集什么、收集了多久，以及你了解到的“用户随着时间的推移是如何与你的产品互动”的知识。请它找出该数据中信号最强的三种行为模式，并设计一个将每种模式转化为系统性模型改进的反馈循环。然后要求它帮你起草一份一页纸的“护城河叙事”，用于指导产品营销：讲述你的数据飞轮是如何运作的，它转了多久，以及为什么一个从今天才开始、资源丰富的竞争对手在不到两年的时间里无法复制它。
...
复利化的数据网络效应使你的产品难以被复制，但用户“工作流锁定”则使你的产品难以被舍弃。用户在日常操作中运行你的产品的时间越长，它就越深地嵌入到他们的实际工作方式中。他们在产品之上构建了自动化，培训员工去使用，并将其连接到他们的数据源和其他工具。他们开发的提示词、他们优化的工作流以及他们标准化的输出结果，都是围绕着“你的产品做什么”以及“它怎么做”而塑造的。到了这一步，更换产品就不再是一个简单的产品决策，而是变成了一个全面规模的运营大工程。
...
创造工作流锁定的第一步，是让 Claude 按照“集成深度”来映射你当前的客户群。对于每个客户群，找出他们在你的产品之上构建了哪些工作流，以及他们依赖哪些集成。这能显示出你的产品在哪里具有粘性，以及它需要向哪里扎得更深。
...
你提供的集成越多，客户依赖你的产品构建工作流的表面积就越大。Claude Code 能帮助你快速构建出与目标用户依赖的数据管道、项目管理工具和其他系统的原生集成。Claude Code 还可以构建 API、Webhooks 和 SDK，让客户不仅能使用你的产品，还能在你的产品之上构建新事物——这是最深层次的锁定形式。
...
* **练习：** 要求 Claude 帮你为前十名核心客户构建一份“工作流集成审计报告”。对于每一个客户，记录他们构建的自动化流程、他们依赖的外部集成、流经你产品的团队工作流，以及你对他们更换产品的转换成本估算。然后要求 Claude 找出这个群体中的规律：对于你的特定产品，什么类型的集成创造了最深层的锁定？以及你可以构建或启用什么新功能，来加深那些目前还停留在表面使用的客户的集成程度。
...
在 AI 时代，创始人的职责并没有改变：寻找一个真实的问题，开发一个解决它的产品，并将其规模化为一家举足轻重的公司。改变的，是通往那个目标的路径。纵观四个阶段——创意、MVP、发布和扩展——AI 将数个季度的时长压缩成了几周。
...
过去需要几个月的验证周期，现在只需要几个下午。一个可用的产品原型不再需要一位精通相关技术栈的联合创始人；它只需要一个清晰的问题，以及与智能体编程工具进行几次专注的对话。产品的发布准备工作从过去“发布前的手忙脚乱”压缩成了持续流畅的工作流。在扩展阶段，过去那些迫使早期员工不得不扮演“救火队员”的繁重运营包袱，现在可以越来越多地移交给 AI，从而解放你的团队，让他们把注意力集中在能成为你护城河的“关键判断”上。
...
* **《为初创企业构建 AI 智能体》：** 分享初创企业如何利用智能体在扩大规模时减少对创始人的依赖。
...
* **Claude Code 文档：** 引导开发者从初始安装进阶到高级智能体工作流。专业提示：从阅读“Claude Code 如何工作”概览开始。
...
* **Claude Code 最佳实践：** 涵盖了在 Anthropic 内部以及各个工程团队中行之有效的模式——上下文管理、权限控制、计划以及验证工作流。
...
* **使用 `CLAUDE.md` 文件：** 逐步介绍如何针对你的特定代码库配置 Claude Code。这是处于 MVP 阶段的创始人搭建开发环境的必读材料。
...
* **Claude Code 进阶用户技巧：** 重点介绍来自 Claude Code 团队自身的工作流模式，包括并行对话会话和验证循环。
...
* **Claude Cowork 入门：** 分享团队如何设置 Claude Cowork，并开始实施能力扩展（skills）、插件及其他功能，以将其影响力扩展到整个初创公司。
* **Tutorials:** `claude.com/resources/tutorials` offers a searchable list of hands-on walkthroughs for specific tasks.
* **教程：** `claude.com/resources/tutorials` 提供了一个针对特定任务的动手实践演练的可搜索列表。
...
* **《三家 YC 初创公司如何使用 Claude Code 打造企业》：** 探讨 HumanLayer (F24)、Ambral (W25) 和 Vulcan Technologies (S25) 是如何使用 Claude 快速将原型推向市场，并通过智能体编程工作流扩展 AI 驱动平台的。
...
* **GC AI** 的创始人利用领域专长构建了一个基于 Claude 驱动、响应迅速的法律平台，该平台契合内部团队的实际工作方式：提供公司专属操作手册、跨部门利益相关者管理，以及可变的风险承受阈值设定。
...
* **Carta Healthcare** 使用 Claude 驱动其临床信息提取平台，每年处理 22,000 个外科病例，并将数据提取时间缩短了 66%。
...
* **Anything** 在 Claude 和 Agent SDK 的驱动下，已帮助 150 万用户在不写代码的情况下将想法转化为可用的软件产品，其中包括一位非技术创始人，他构建并已经开始销售一个完整的招聘平台。Anything 的 AI 智能体负责全部的构建工作，因此独立创业者（solopreneurs）可以加倍专注于自己的领域专长。
...
* **Cogent** 是一家应用 AI 实验室，致力于构建自动化执行关键企业安全任务的智能体。这家初创公司将 Claude 作为智能体的推理层，从而在整个漏洞生命周期中实现调查、优先级排序和补救措施的自动化。
...
* **Airtree** 将 Claude Cowork 作为其运营基础设施的核心，统一了过去散落在十几个不同工具和团队中的数据。现在，只要一个人利用扩展能力（skills）构建了某项工作流自动化，组织中的每个人就都可以利用它来完成那些一直停留在待办清单上未被完成的事务。
...
* **Duvo** 构建了能在 ERP 系统、供应商门户、电子表格、电子邮件甚至电话中运行采购、供应链和品类管理流程的 AI 智能体。Duvo 完全基于 Claude 构建，使用 Agent SDK 来调度跨系统的工作流。
...
* **Zingage** 是一个专为家庭护理机构提供全天候 24/7 自动化运营的 AI 智能体平台。这家初创公司利用 Claude 的结构化工具调用（tool calling）在电子病历（EMR）和多个沟通渠道之间进行协调，并利用 Claude 的上下文推理能力构建智能体，这些智能体能够提供细致入微、量身定制的患者服务结果，而不是只会匹配最常见的话术回应。
...
* **Kindora** 是一个由非营利组织高管构建的 AI 驱动平台，他利用 Claude Sonnet 构建了一个急需的工具，用于智能匹配慈善机构与资助者。在将成千上万的潜在匹配筛选至极少数值得争取的对象后，Kindora 的 MCP 连接器允许非营利组织直接在 Claude 内部访问其潜在客户挖掘工具。
...
* **Wordsmith** 由一位由律师转型的 CTO 创立，旨在为企业内部法务团队提供可靠的 AI 法律技术。Claude 是 Wordsmith 合同审查、协议起草和文档审查能力的推理引擎，而且该初创公司的工程团队本身也使用 Claude Code 来构建和演进该平台。
...
* **Anthropic 初创企业计划：** 面向与 Anthropic 风投合作伙伴合作的初创公司，该计划提供免费的 API 额度、最高级别的公共可用速率限制，以及参加独家创始人活动和研讨会的邀请。
* **Claude community:** Forums and community spaces for builders.
* **Claude 社区：** 供开发者交流的论坛和社区空间。
* **Live learning resources:** Conferences, webinars, livestreams, and recordings.
* **实时学习资源：** 会议、网络研讨会、直播和录像回放。
